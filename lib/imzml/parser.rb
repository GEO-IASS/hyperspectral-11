	...
	class Parser < ::Ox::Sax
		...
		def attr(name, str)
			...
			when :value
				...
				if @in_scan_settings
					case accession
					when OBO::IMS::MAX_COUNT_OF_PIXELS_X then @metadata.pixel_count_x = value.to_i
					when OBO::IMS::MAX_COUNT_OF_PIXELS_Y then @metadata.pixel_count_y = value.to_i
					when OBO::IMS::PIXEL_SIZE then @metadata.pixel_size_x = 1 # FIXME not used properly value.to_i
					when OBO::IMS::IMAGE_SHAPE then @metadata.pixel_size_y = 1 # value.to_i # FIXME probably error in obo definition file, should be pixel size y
					# when OBO::IMS::LINESCAN_SEQUENCE_BOTTOM_UP then @metadata.linescan_sequence = accession.to_s
					# when OBO::IMS::LINESCAN_SEQUENCE_BOTTOM_UP then @metadata.linescan_sequence = accession.to_s
					# when OBO::IMS::LINESCAN_SEQUENCE_TOP_DOWN then @metadata.linescan_sequence = accession.to_s
					# when OBO::IMS::LINESCAN_SEQUENCE_LEFT_RIGHT then @metadata.linescan_sequence = accession.to_s
					# when OBO::IMS::LINESCAN_SEQUENCE_RIGHT_LEFT then @metadata.linescan_sequence = accession.to_s
					# when OBO::IMS::LINESCAN_SEQUENCE_NO_DIRECTION then @metadata.linescan_sequence = accession.to_s
					# when OBO::IMS::SCAN_PATTERN_MEANDERING then @metadata.scan_pattern = accession.to_s
					# when OBO::IMS::SCAN_PATTERN_RANDOM_ACCESS then @metadata.scan_pattern = accession.to_s
					# when OBO::IMS::SCAN_PATTERN_FLYBACK then @metadata.scan_pattern = accession.to_s
					# when OBO::IMS::SCAN_TYPE_HORIZONTAL_LINE_SCAN then @metadata.scan_type = accession.to_s
					# when OBO::IMS::SCAN_TYPE_VERTICAL_LINE_SCAN then @metadata.scan_type = accession.to_s
					# when OBO::IMS::LINE_SCAN_DIRECTION_LINESCAN_RIGHT_LEFT then @metadata.line_scan_direction = accession.to_s
					# when OBO::IMS::LINE_SCAN_DIRECTION_LINESCAN_LEFT_RIGHT then @metadata.line_scan_direction = accession.to_s
					end
				end
			
				if @in_spectrum && @in_scan_list && @in_scan
					case accession
					when OBO::IMS::SPECTRUM_POSITION_X then @current_position_x = value.to_i - 1
					when OBO::IMS::SPECTRUM_POSITION_Y then @current_position_y = value.to_i - 1
					end
				end

				if @in_referenceable_param_group && @in_reference_param_group_list
					if accession == OBO::MS::FLOAT_32_BIT
						# p case
				...
				if @in_file_description && @in_file_content
					case accession
					when OBO::IMS::UNIVERSALLY_UNIQUE_IDENTIFIER
						uuid = value.delete("{}-")
						@metadata.uuid = uuid
					when OBO::IMS::CONTINUOUS then @metadata.saving_type = accession
					when OBO::IMS::PROCESSED then @metadata.saving_type = accession
					end
				end
				...
				if @in_spectrum_list && @in_spectrum && @in_binary_data_array_list
					...
					# FIXME
					case accession
					when OBO::IMS::EXTERNAL_OFFSET then @spectrum.send("#{type}_array_external_offset=", value)
					when OBO::IMS::EXTERNAL_ENCODED_LENGTH then @spectrum.send("#{type}_array_external_encoded_length=", value)
					end
				end
		...
		def end_element(name)
			...
			when :spectrum
				...
				if @metadata.saving_type == OBO::IMS::CONTINUOUS
